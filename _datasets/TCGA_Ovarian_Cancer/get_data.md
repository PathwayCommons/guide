...
This section provides detailed instructions on sourcing gene expression data suitable for downstream differential expression analysis (Figure 1). Though the focus here is on obtaining gene expression data the methods can be easily adapted to other types of available data. By then end of this discussion you should:

1. Be familiar with the GDC data repository
2. Be able to source RNA sequencing data from a TCGA study
...
The preceding R code is presented again in its entirety.
...
- TCGA-OV RNA-seq expression data (counts) and subtypes (group) inside a DGEList
  - Download: [TCGAOV_data.rda]({{ site.baseurl }}/{{ site.media_root }}{{ page.id }}/{{ page.data.tcgaov_data }})
    - variable name: `TCGAOV_data`
    - type: edgeR DGEList container
    - format: Rdata
    - size: 1.9 MB
