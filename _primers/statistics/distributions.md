...
#### Poisson-gamma notation in RNA-seq

This mixture model will be important in our discussion of RNA-seq data [differential expression testing]({{ site.baseurl }}/datasets/TCGA_Ovarian_Cancer/process-data/#differentialExpression). In this case the notation is altered where $$r^-1=\alpha^{-1}=\beta^{-1}=\phi$$ is the 'dispersion' for some set of RNA species counts $$Y$$.

$$
\begin{equation*}
  \begin{split}
    Y &\sim NB \left(\phi^{-1}, \frac{\lambda}{\phi^{-1} + \lambda}\right)\\
    f(y; \lambda,\phi^{-1}) &=  \binom{y+\phi^{-1}-1}{y} \left(\frac{\lambda}{\phi^{-1} + \lambda} \right)^y
     \left(\frac{\phi^{-1}}{\phi^{-1} + \lambda} \right)^{\phi^{-1}} \\    
  \end{split}
\end{equation*}
$$

Sometimes in the context of RNA-seq model fitting, you might come across the negative binomial parametrized by its mean and variance.
