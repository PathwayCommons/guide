...
There will be $$x+r$$ total trials but the last event is a failure so we really care about the first $$x+r-1$$ trials. There will be $$x$$ successes and $$(r-1)$$ failures in any order. Each order has a probability identical to a binomial trial $$p^c(1-p)^r$$.

$$
\begin{equation*}
  \begin{split}
    f(x) &=  \binom{x+r-1}{x} p^x(1-p)^r \text{  for }x=0,1,2,\cdots\\
  \end{split}
\end{equation*}
$$
...
Count data such as RNA sequencing mapped reads is often modeled with a Poisson distribution where the mean and variance are equal to $$\mu$$. However, there are cases where the variance exceeds that specified by the mean. To account for this 'over-dispersed' data, the negative binomial distribution can be utilized. As we will show below, the negative binomial arises as a Poisson distribution where the Poisson parameter is itself a random variable distributed according to a Gamma distribution.

Let us state this in a more precise fashion. Suppose that we have distribution of counts $$X$$ that follows a Poisson distribution indexed by the parameter $$\Theta$$. Now suppose that $$\Theta$$ is itself some function of another random variable $$\Theta = \lambda\epsilon$$ where $$\epsilon \sim Gamma(x;\alpha,\beta)$$. Then the conditional distribution of the random variable of counts is

$$
\begin{equation*}
  P(X=x | \Theta = \lambda\epsilon) = \frac{(\lambda\epsilon)^x}{x!}e^{- (\lambda\epsilon)} \text{  for } x=0,1,2,\cdots
\end{equation*}
$$
...
The join density of $$N$$ and $$\Theta$$ is

$$
\begin{equation*}
  \begin{split}
    P(X=x | \Theta = \lambda\epsilon) \cdot g(\epsilon) &= \frac{(\lambda\epsilon)^x}{x!}e^{- \lambda\epsilon} \cdot \frac{\beta^\alpha}{\Gamma(\alpha)}\epsilon^{\alpha-1}e^{-\beta \epsilon}\\
  \end{split}
\end{equation*}
$$
...
\begin{equation*}
  \begin{split}
    P(X=x) &= \int_{0}^\infty \frac{(\lambda\epsilon)^x}{x!}e^{- \lambda\epsilon} \cdot \frac{\beta^\alpha}{\Gamma(\alpha)}\epsilon^{\alpha-1}e^{-\beta \epsilon} d\epsilon\\

    &= \frac{\lambda^x\beta^\alpha}{x! \Gamma(\alpha)} \int_{0}^\infty \epsilon^{x+\alpha-1}  e^{-(\beta + \lambda)\epsilon} d\epsilon\\    
  \end{split}
\end{equation*}
$$

The key here is to transform the integrand into a gamma distribution with shape parameter $$x+\alpha$$ and scale $$\beta+\lambda$$ and noting that the integral over all values is unity.

$$
\begin{equation*}
  \begin{split}

    P(X=x) &= \frac{\lambda^x \beta^\alpha}{x! \Gamma(\alpha)} \frac{\Gamma(x+\alpha)}{(\beta + \lambda)^{x + \alpha}} \int_{0}^\infty \frac{(\beta + \lambda)^{x + \alpha}}{\Gamma(x+\alpha)} \epsilon^{x+\alpha-1}  e^{-(\beta + \lambda)\epsilon} d\epsilon \\

          &= \frac{\Gamma(x+\alpha)}{x! \Gamma(\alpha)} \frac{\lambda^x  \beta^\alpha}{(\beta + \lambda)^{x + \alpha}} \\

          &= \frac{(x+\alpha-1)!}{x! (\alpha - 1)!} \frac{\lambda^x}{(\beta + \lambda)^x} \frac{\beta^\alpha}{(\beta + \lambda)^{\alpha}} \\

          &= \binom{x+\alpha-1}{x} \left(\frac{\lambda}{\beta + \lambda }\right)^x \left(\frac{\beta}{\beta + \lambda}\right)^\alpha  \text{ for }x=0,1,\cdots
  \end{split}
\end{equation*}
$$

It is simple to see that this result is the negative binomial with $$r=\alpha$$ and $$p=\lambda/(\beta+\lambda)$$. In this case the moments can be stated using these new variables.
...
\begin{equation*}
  \begin{split}
    E[X] &= \left(\frac{\lambda}{\beta + \lambda}\right) \left(\frac{\beta + \lambda}{\beta}\right) \alpha\\    
...
\begin{equation*}
  \begin{split}
    Var[X] &= \left(\frac{\lambda}{\beta + \lambda}\right) \left(\frac{\beta + \lambda}{\beta}\right)^2 \alpha\\    
         &= \frac{\alpha \lambda}{\beta} \left(1 + \frac{\lambda}{\beta} \right)\\    
  \end{split}
\end{equation*}
$$

We'll see this relationship in our discussion of RNA-seq data [differential expression testing]({{ site.baseurl }}/datasets/TCGA_Ovarian_Cancer/process-data/#differentialExpression) where $$\alpha^{-1}=\beta^{-1}=\phi$$ is the 'dispersion' and $$X \equiv Y$$.
...
Note that as the dispersion parameter $$phi$$ approaches zero the negative binomial variance approaches the Poisson. Thus the dispersion parameter accounts for the extra variability compared to the the Poisson.
