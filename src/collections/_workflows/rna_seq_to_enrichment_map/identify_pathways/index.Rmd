...
layout: page
data:
  gene_set_database: Human_GOBP_AllPathways_noPFOCR_no_GO_iea_April_01_2024_symbol.gmt
  report_brca: gsea_report_for_brca.tsv
  report_hd: gsea_report_for_hd.tsv
...
<div class="card bg-light">
  <div class="card-body">
    <p class="card-text">
      <strong>Figure 4. Gene set database excerpt</strong>. Gene sets are defined in rows where the first column is the name, the second is a description and gene names within the set occupy the remaining row entries. Highlighted in blue is the NetPath IL-5 signal transduction pathway that includes 59 gene names.  <a href="{{ page.figures.figure_4 }}" target="_blank">Open in new window</a>
    </p>
  </div>
</div>
...
<div class="card bg-light mb-3">
  <div class="card-body">
    <h3 class="card-title">Box 1. GSEA steps</h3>
    <p class="card-text">
      <ol>
        <li>
...
1. [Register](http://software.broadinstitute.org/gsea/register.jsp){: target="_blank"} to use the GSEA software
2. [Login](http://software.broadinstitute.org/gsea/login.jsp){: target="_blank"}
3. [Download](http://software.broadinstitute.org/gsea/downloads.jsp){: target="_blank"}
...
<div class="card bg-light">
  <div class="card-body">
    <p class="card-text">
      <strong>Figure 5. The GSEA interface.</strong> The control panel on the left (e.g. <code>Steps in GSEA analysis</code>) provides quick access to the most common actions. The main window displays the <code>Home</code> tab by default. Each control panel action typically opens a new tab in the main window. <a href="{{ page.figures.figure_5 }}" target="_blank">Open in new window</a>
    </p>
  </div>
</div>
### Input
...
<a href="{{ site.baseurl }}{{ process_data.url | remove: 'index.html' }}{{ process_data.data.rank }}" role="button" class="btn btn-secondary btn-lg btn-block" download>Ranks (.rnk)</a>


#### Gene set database

This is the human gene set database culled from various sources, [generated by the Bader lab](https://download.baderlab.org/EM_Genesets/).

<a href="{{ page.data.gene_set_database }}" role="button" class="btn btn-secondary btn-lg btn-block" download>Gene Set Database (.gmt)</a>
...
<div class="card bg-light">
  <div class="card-body">
    <p class="card-text">
      <strong>Figure 9. Enrichment plot for NetPath IL-5 signalling pathway.</strong> Shown is a the running sum plot for the IL-5 signal transduction pathway described earlier. <a href="{{ page.figures.figure_9 }}" target="_blank">Open in new window</a>
    </p>
  </div>
</div>

- **Detailed enrichment results.** These are the end-goal for our workflow step! These files provide a summary report of gene sets enriched in this phenotype. You can find these files inside your GSEA results folder named `gsea_report_for_na_pos_XXXXXXXXXXXXX.tsv` and `gsea_report_for_na_neg_XXXXXXXXXXXXX.tsv`. The rows of the report include information for each enriched gene set (Table 2).

  **Table 2. Excerpt of enrichment report for BrCa class**
...
We have renamed the enrichment report files and provide them below.
  ...
  <a href="{{ page.data.report_brca }}" role="button" class="btn btn-secondary btn-lg btn-block" download>BrCa report (.tsv)</a>

  2. Enrichment report for HD
  <a href="{{ page.data.report_hd }}" role="button" class="btn btn-secondary btn-lg btn-block" download>HD report (.tsv)</a>
