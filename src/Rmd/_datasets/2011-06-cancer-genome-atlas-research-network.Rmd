...
A statistical method called *Factor Analysis* was employed to create a unified expression measure for each gene and each patient sample represented on all three platforms (Wang 2011). Combining measures in this way tends to mitigate platform-specific contributions and simplifies the downstream measurements compared to dealing with many data sets. In essence, the unified expression measure for each gene/sample is a weighted contribution from each platform.

Let $$x_{gij}$$ be the observed expression for gene $$g$$ in sample $$s$$ for microarray platform $$p$$. This observed measurement arises from a platform-independent, underlying factor $$f_{gs}$$ representing the unobserved gene expression and a platform-specific contribution $$u_{sp}$$ or noise. A coefficient $$\lambda_{p}$$ is called a 'loading' and represents the correlation between the factor and observed expression variable. The factor model describing the observed expression (omitting gene symbol $$g$$) is

$$
\begin{equation*}
  \begin{split}
      x_{sp} &= \lambda_{p}f_{s} + u_{sp}      
  \end{split}
\end{equation*}
$$

Under some mild assumptions about $$f$$ and $$u$$, the goal is to determine the value of this unified expression measure of a gene in a sample $$f_{s}$$ using observed expression data.
